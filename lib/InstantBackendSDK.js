...
class InstantBackend {
   ...
   */
  async register({ username, password, email, fullName, customAttributes }) {
    try {
  ...
  async listAccountUsers() {
    try {
      const response = await fetch(`${this.baseUrl}/account/users`, {
        method: 'GET',
      ...
      if (!response.ok) {
        this.throwResponseError('Failed to list account users', response);
      }
  ...
  async requestPasswordReset({ email, username }) {
    try {
      ...
      if (!response.ok) {
        this.throwResponseError('Failed to request password reset', response);
      }
  ...
  async resetPassword({ token, password }) {
    try {
      ...
      if (!response.ok) {
        this.throwResponseError('Failed to reset password', response);
      }
  ...
  async changePassword({ currentPassword, newPassword }) {
    try {
      const response = await fetch(`${this.baseUrl}/account/password/change`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
      ...
      if (!response.ok) {
        this.throwResponseError('Failed to change password', response);
      }
  ...
  async changeAccountUserPassword({ id, password }) {
    try {
      const response = await fetch(`${this.baseUrl}/account/users/${id}/password`, {
        method: 'PATCH',
        headers: this.getAuthHeaders(),
      ...
      if (!response.ok) {
        this.throwResponseError('Failed to change account user password', response);
      }

      return await response.json();
    } catch (error) {
      console.error('Change account user password error:', error);
      throw error;
    }
  }
...
class Collection {
  ...
  async update(id, data) {
    try {
      ...
      if (!response.ok) {
        this.instantBackend.throwResponseError('Failed to update document', response);
      }
  ...
  async delete(id) {
    try {
      ...
      if (!response.ok) {
        this.instantBackend.throwResponseError('Failed to delete document', response);
      }
